...
class Automato:
    ...
    def remover_estados_inuteis(self):
        confirmar = self.automato_determinismo()
        if(confirmar):
            print("Então pode usar a função!")
            estados_inuteis = set()
            estados_uteis = []
            estados_finais = set(self.estados_finais)
            # Adiciona o estado inicial à lista de estados úteis
            ...
            # Percorre todas as transições até encontrar todos os estados úteis
            ...
            while True:
                ...
                for estado_atual in temp_estados:
                    for simbolo in self.transicoes[estado_atual]:
                        if (estado_atual is not estados_uteis or estado_atual in self.estados_finais) and agora in self.transicoes[estado_atual][simbolo]:
                            continua = False
                            agora = estado_atual
    ...
    def gerar_automato(self):
        self.estados = set()
        self.alfabeto = self.simbolos.copy()
        self.transicoes = dict()
        self.estados_finais = set()

        variavel_estado = {}
        for variavel in self.variaveis:
            variavel_estado[variavel] = set()

        estado_inicial = self.inicial
        variavel_pilha = [estado_inicial]
        estado_contador = 0
        variavel_estado[estado_inicial].add(estado_contador)
        self.estados.add(estado_contador)

        while variavel_pilha:
            variavel = variavel_pilha.pop()
            for producao in self.producao[variavel]:
                novo_estado = estado_contador + 1
                if producao == "ε":
                    self.estados_finais.add(estado_contador)
                else:
                    variavel_pilha.append(producao[-1])
                    if producao[-1] in variavel_estado:
                        variavel_estado[producao[-1]].add(novo_estado)
                    else:
                        variavel_estado[producao[-1]] = {novo_estado}
                        self.estados.add(novo_estado)

                    transicao = (estado_contador, producao[0])
                    if transicao in self.transicoes:
                        self.transicoes[transicao].add(novo_estado)
                    else:
                        self.transicoes[transicao] = {novo_estado}

                estado_contador += 1

        self.estado_inicial = list(variavel_estado[estado_inicial])[0]

    def gerar_automato_finito_deterministico(self):
        auto = Automato()
        variaveis_novo = []
        auto.alfabeto = self.simbolos
        auto.estado_inicial = self.inicial
        variaveis_pilha = [self.inicial]

        while len(variaveis_pilha) > 0:
            variavel_atual = variaveis_pilha.pop(0)
            if variavel_atual not in variaveis_novo:
                variaveis_novo.append(variavel_atual)
                for producao in self.producao[variavel_atual]:
                    if producao.isupper():
                        variaveis_pilha.append(producao)
                    else:
                        if variavel_atual not in auto.transicoes:
                            auto.transicoes[variavel_atual] = {}
                        if producao not in auto.transicoes[variavel_atual]:
                            auto.transicoes[variavel_atual][producao] = [variavel_atual + producao[1:]]

        auto.estados = variaveis_novo
        auto.estados_finais = [estado for estado in auto.estados if estado.islower()]
        self = auto
...
automato.imprimir_automato()
automato.remover_estados_inuteis()
automato.imprimir_automato()
automato.salvar_gramatica(automato.gerar_gramatica_linear_direita(automato), "temp")

automato3 = Automato()
automato3.carregar_gramatica_arquivo("gramatica.txt.txt")
automato3.imprimir_gramatica()
automato3.gerar_automato_deterministico()
automato3.imprimir_automato()
