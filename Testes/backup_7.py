        ...
        for transicao_temp in self.estados:
            if "ε" in self.transicoes[transicao_temp]:
                ...
                for i in range(len(self.estados)):
                    if transicao_temp == self.estados[i]:
                        self.estados.pop(i)
                for i in range(len(self.estados_finais)):
                    if transicao_temp == self.estados_finais[i]:
                        self.estados_finais.pop(i)

    def verificar_estados_inacessiveis(self):
        estados_acessiveis = set()
        fila = [self.estado_inicial]

        while fila:
            estado_atual = fila.pop(0)
            estados_acessiveis.add(estado_atual)

            for simbolo in self.simbolos:
                estado_proximo = self.transicoes.get((estado_atual, simbolo), None)
                if estado_proximo and estado_proximo not in estados_acessiveis:
                    fila.append(estado_proximo)

        estados_inacessiveis = self.estados - estados_acessiveis
        if estados_inacessiveis:
            print(f"Existem estados inacessíveis: {estados_inacessiveis}")
            opcao = input("Deseja removê-los? (S/N)").strip().upper()
            if opcao == "S":
                self.estados -= estados_inacessiveis
                for estado_inacessivel in estados_inacessiveis:
                    for simbolo in self.simbolos:
                        if (estado_inacessivel, simbolo) in self.transicoes:
                            del self.transicoes[(estado_inacessivel, simbolo)]
                print("Estados inacessíveis removidos com sucesso!")
